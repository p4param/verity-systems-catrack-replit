...
import { ConciergeBell, ChevronRight, Plus, RefreshCw, Search } from "lucide-react";
import { ServiceStyleLookupItem } from "@/modules/cat/service-styles/types";

interface ServiceStyleLookupProps {
  value?: ServiceStyleLookupItem | string | null;
  onChange: (serviceStyle: ServiceStyleLookupItem | null) => void;
  placeholder?: string;
  disabled?: boolean;
...
export default function ServiceStyleLookup({
  value,
  onChange,
  ...
  onRequestCreate,
}: ServiceStyleLookupProps) {
  const [query, setQuery] = useState("");
  const [options, setOptions] = useState<ServiceStyleLookupItem[]>([]);
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setLoading(true);
      try {
        const res = await fetch(`/api/cat/service-styles/lookup?query=${encodeURIComponent(query.trim())}`);
        ...
        console.error("Service style lookup load failed:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleQuickCreate = async () => {
    ...
    try {
      ...
      if (!res.ok || !json?.success) {
        alert(json?.error || "Unable to create service style master.");
        return;
      }

      const created = json.serviceStyle || { id: json.id, name: styleName };
      onChange({
        id: created.id,
        name: created.name || styleName,
        code: created.code || "",
        showInDiscoveryQuickSelect: false,
      ...
      console.error("Service style quick create failed:", err);
      alert("Unable to create service style master.");
    } finally {
      setCreating(false);
    }
  };
      ...
      <div className="bg-gradient-to-r from-primary/10 via-primary/5 to-card border border-primary/30 p-3.5 rounded-2xl flex items-center justify-between gap-3 text-xs shadow-xs transition hover:border-primary/50">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[11px] text-muted-foreground font-medium">Primary Dining Format Master</div>
          </div>
        </div>
        <button
          type="button"
    ...
    <div ref={wrapperRef} className="space-y-2 relative">
      ...
      {open && (
        <div className="absolute z-20 left-0 right-0 mt-1 bg-card border border-border/60 rounded-xl shadow-xl max-h-72 overflow-y-auto divide-y divide-border/30">
          {loading ? (
            <div className="p-3 text-[11px] text-muted-foreground">Loading service styles...</div>
          ) : noResults ? (
            <div className="p-3 text-[11px] text-muted-foreground space-y-2">
              <div>No matching service styles found.</div>
              {allowQuickCreate && query.trim() && (
                ...
                >
                  <span className="flex items-center gap-1.5 truncate">
                    ...
                    <span>Create Service Style &quot;{query.trim()}&quot;</span>
                  </span>
