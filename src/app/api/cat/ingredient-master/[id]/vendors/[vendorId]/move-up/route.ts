...
// PM-WP04A — Move Up: swaps this Vendor's Priority with whoever holds
// the adjacent (numerically lower) rank. No request body.
...
export async function POST(req: NextRequest, props: any) {
  try {
    ...
    const result = await moveVendorPriority(tenantId, userId, id, vendorId, 'up');
    if (!result.ok) {
    ...
    console.error('Error moving Vendor Priority up:', error);
