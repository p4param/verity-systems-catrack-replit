/**
 * @deprecated LEGACY — Event Types API.
 *
 * This route (GET/POST /api/events/masters/types) is a prototype-era implementation.
 * It is superseded by the Core Master Data Engine (MDE) at /api/masters/categories.
 *
 * STATUS: Operational but deprecated. The CateringEventType Prisma model still has
 * active FK relations from CateringEvent. Do NOT drop the table until MDE migration
 * and data transfer are complete.
 *
 * Migration target: /api/masters/categories?module=EVENT&key=EVENT_TYPE
 * Prisma model: CateringEventType → catering_event_types table
 */
...
const DEPRECATION_HEADER = {
  ...
  "X-Migration-Target": "/api/masters/categories?module=EVENT&key=EVENT_TYPE",
};
...
export async function GET(req: Request) {
  try {
    ...
    const items = await prisma.cateringEventType.findMany({
...
export async function POST(req: Request) {
  try {
    ...
    const item = await prisma.cateringEventType.create({
