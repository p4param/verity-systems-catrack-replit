/**
 * @deprecated LEGACY — Event Category update/delete.
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await prisma.cateringEventCategory.findFirst({
    ...
    const item = await prisma.cateringEventCategory.update({
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.cateringEventCategory.update({
      where: { id: existing.id },
