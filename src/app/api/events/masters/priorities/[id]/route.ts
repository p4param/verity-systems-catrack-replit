/**
 * @deprecated LEGACY — Event Priority update/delete.
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    requirePermission(req, "INVENTORY_MASTER_UPDATE");
    const { id } = await params;
    const { name, code } = await req.json();
    const item = await prisma.cateringEventPriority.update({
      where: { id },
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await params;
    await prisma.cateringEventPriority.update({
      where: { id },
