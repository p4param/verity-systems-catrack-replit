...
export default function StockConditionFormModal({ isOpen, onClose, initialData, onSaved }) {
    const { fetchWithAuth, user } = useAuth();
    const isEdit = !!initialData;
    const canUpdate = user?.permissions?.includes("INVENTORY_MASTER_UPDATE");

    // Core ledger states cannot be renamed or fully disabled
    const isCoreState = initialData?.code === "CLEAN" || initialData?.code === "DIRTY";
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEdit) {
                await fetchWithAuth(`/api/masters/stock-conditions/${initialData.id}`, {
                    method: "PUT",
                    body: JSON.stringify(formData)
                });
            } else {
                await fetchWithAuth("/api/masters/stock-conditions", {
                    method: "POST",
                    body: JSON.stringify(formData)
                });
            }
            ...
            <div className="bg-card rounded-lg shadow-xl w-full max-w-md overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-border">
                    <h2 className="text-lg font-semibold text-foreground">
                        {isReadOnly ? "Condition Details" : isEdit ? "Edit Condition" : "New Stock Condition"}
                    </h2>
                ...
                <form onSubmit={handleSubmit} className="p-6">
                    ...
                    {isCoreState && (
                        <div className="mb-4 p-3 bg-blue-50 text-blue-800 rounded text-sm shrink-0">
                            This is a core system condition required for ledger integrity. Read-only fields cannot be altered.
                        </div>
                    )}
