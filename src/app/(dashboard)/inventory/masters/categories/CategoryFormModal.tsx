...
export default function CategoryFormModal({ isOpen, onClose, initialData, onSaved }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEdit) {
                await fetchWithAuth(`/api/masters/categories/${initialData.id}`, {
                    method: "PUT",
                    body: JSON.stringify(formData)
                });
            } else {
                await fetchWithAuth("/api/masters/categories", {
                    method: "POST",
                    body: JSON.stringify(formData)
                });
            }
            ...
            <div className="bg-card rounded-lg shadow-xl w-full max-w-md overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-border">
                    <h2 className="text-lg font-semibold text-foreground">
                         {isReadOnly ? "Category Details" : isEdit ? "Edit Category" : "New Apparel Category"}
                    </h2>
