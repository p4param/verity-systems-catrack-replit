...
import { useCreateField, useUpdateField } from "@/modules/platform/configuration/hooks/use-fields";
...
export function FieldDialog({ open, onOpenChange, entityId, fieldId, initialData }: FieldDialogProps) {
  ...
  const { register, handleSubmit, reset, watch, control, setValue, formState: { errors } } = useForm({
    ...
    defaultValues: {
      ...
      filterable: false,
      displayOrder: 0,
      lookupDefinition: null as any
    }
  });

  const dataType = watch("dataType");
  const uiControl = watch("uiControl");
  const dataSource = watch("dataSource");
  const labelValue = watch("label");

  useEffect(() => {
    if (isNew && labelValue) {
      const generatedCode = labelValue.toUpperCase().replace(/\s+/g, '_').replace(/[^A-Z0-9_]/g, '');
      setValue("code", generatedCode, { shouldValidate: !!errors.code });
    }
  }, [labelValue, isNew, setValue, errors.code]);

  // Adjust defaults when data type or data source changes
  useEffect(() => {
    if (dataType && dataSource) {
      const compatible = FieldControlRegistry.getCompatibleControls(dataType, dataSource)
        .filter(c => c.status !== "Planned");
      const allowedControls = compatible.map(c => c.id);
      if (!allowedControls.includes(uiControl) && allowedControls.length > 0) {
        setValue("uiControl", allowedControls[0]);
      }
    }
  }, [dataType, dataSource]);
    ...
    if (open) {
      if (isNew) {
        ...
        reset({
          ...
          uiControl: initialData.uiControl || "TEXT_INPUT",
          ...
          lookupDefinition: initialData.lookupDefinition || null,
  ...
  const onSubmit = async (data: any) => {
    try {
      if (!RuntimeRegistry.isLookupField(data)) {
        data.lookupDefinition = null;
      }
  ...
  // Calculate compatible controls based on current data type and data source
  const compatibleControls = FieldControlRegistry.getCompatibleControls(dataType, dataSource)
    .filter(c => c.status !== "Planned");
  
  // Create grouped controls for the dropdown
  const groupedControls = compatibleControls.reduce((acc, control) => {
    const category = control.designer.category;
    if (!acc[category]) acc[category] = [];
    acc[category].push(control);
    return acc;
  }, {} as Record<string, typeof compatibleControls>);

  const isLookup = RuntimeRegistry.isLookupField({ dataSource });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[850px] bg-card text-card-foreground max-h-[95vh] overflow-hidden flex flex-col p-0 gap-0">
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col h-full overflow-hidden">
                ...
                <TabsContent value="validation" className="space-y-4 m-0 pb-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input type="number" {...register("metadata.minLength", { setValueAs: (v) => v === "" || isNaN(v) ? null : Number(v) })} className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm" placeholder="e.g. 5" />
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-muted-foreground mb-1">Maximum Length / Value</label>
                      <input type="number" {...register("metadata.maxLength", { setValueAs: (v) => v === "" || isNaN(v) ? null : Number(v) })} className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm" placeholder="e.g. 100" />
                    </div>
                  </div>
                </TabsContent>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="col-span-2 grid grid-cols-3 gap-4 border p-4 rounded-lg border-border bg-muted/10">
                      <div>
                        ...
                        <input type="number" {...register("metadata.width", { setValueAs: (v) => v === "" || isNaN(v) ? null : Number(v) })} className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm" placeholder="e.g. 100" />
                      </div>
                      <div>
                ...
                <TabsContent value="advanced" className="space-y-4 m-0 pb-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="number" {...register("displayOrder", { setValueAs: (v) => v === "" || isNaN(v) ? 0 : Number(v) })} className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm" placeholder="e.g. 10" />
                    </div>
                  </div>
          ...
          <DialogFooter className="p-4 pt-4 border-t border-border bg-muted/10 mt-auto shrink-0 flex flex-col items-end gap-2">
            {Object.keys(errors).length > 0 && (
              <div className="w-full text-xs text-rose-500 bg-rose-500/10 p-2 rounded">
                Validation errors in: {Object.keys(errors).join(', ')}
              </div>
            )}
