...
const DEFAULT_FORM: OccasionTypeForm = {
  name: "",
  code: "",
  isActive: true,
...
export default function OccasionTypesPage() {
  ...
  const [formData, setFormData] = useState<OccasionTypeForm>(DEFAULT_FORM);
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      console.error("Error loading occasion types:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !json?.success) {
        alert(json?.error || "Failed to save occasion type.");
        return;
      }
      ...
      console.error("Save occasion type error:", err);
      alert("Unexpected error saving occasion type.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Are you sure you want to delete occasion type "${name}"?`)) return;
    try {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border/40 pb-5">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">
            ...
            <span>/</span>
            <span className="text-foreground">Occasion Types</span>
          </div>
          <h1 className="text-2xl font-black text-foreground flex items-center gap-2">
            <Sparkles className="w-6 h-6 text-primary shrink-0" />
            <span>Occasion Types Master</span>
          </h1>
          <p className="text-xs text-muted-foreground mt-0.5">
            Configure occasion masters for discovery workflows and quick select options.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            placeholder="Search occasion types by name or code..."
      ...
      <div className="bg-card border border-border/40 rounded-2xl overflow-hidden shadow-xs">
        {loading ? (
          <div className="p-8 text-center text-xs text-muted-foreground">
            Loading occasion types...
          </div>
        ) : items.length === 0 ? (
          <div className="p-12 text-center space-y-2">
            <Sparkles className="w-8 h-8 text-muted-foreground/40 mx-auto" />
            <div className="text-sm font-bold text-foreground">No Occasion Types Found</div>
            <div className="text-xs text-muted-foreground max-w-sm mx-auto">
              Get started by adding occasion types to power discovery quick chips and master lookup dropdowns.
            </div>
          </div>
        ) : (
                ...
                <tr key={item.id} className="hover:bg-muted/30 transition">
                  ...
                  <td className="p-3 text-center">
                    ...
                    >
                      ...
                      <span>{item.showInDiscoveryQuickSelect ? "Quick Select" : "Standard Lookup"}</span>
                    </button>
                  </td>
                  <td className="p-3 text-center">
                    <button
          ...
          >
            <div className="border-b border-border/40 pb-3 flex items-center justify-between">
              <h3 className="text-base font-bold text-foreground">
                {editingId ? "Edit Occasion Type" : "Add Occasion Type"}
              </h3>
              <button
                type="button"
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="space-y-1">
                <label className="font-bold text-foreground block">Code (Optional)</label>
                <input
                  type="text"
                  value={formData.code}
                  onChange={(e) => setFormData((prev) => ({ ...prev, code: e.target.value }))}
                  placeholder="Auto-generated if left blank"
                  className="w-full bg-background border border-border/60 rounded-xl px-3 py-2 text-xs focus:ring-1 focus:ring-primary font-mono uppercase"
                />
              </div>
              ...
              <div className="pt-2 space-y-2">
                ...
                <p className="text-[11px] text-muted-foreground pl-6">
                  If enabled, this occasion will appear as a quick choice chip in the Event Basics workspace.
                </p>
              </div>
